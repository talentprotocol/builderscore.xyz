...
import { fetchNotificationTokens, processNotificationTokensData, calculateCumulativeData } from "@/app/services/neynar";
import { NotificationTokensApiResponse, NotificationTokensParams, NotificationToken } from "@/app/types/neynar";

export async function GET(request: NextRequest): Promise<NextResponse<NotificationTokensApiResponse>> {
  try {
    ...
    return NextResponse.json(
      { 
        ...
        data: [],
        cumulativeData: [],
        error: error instanceof Error ? error.message : "Failed to fetch notification tokens" 
      },
      { status: 500 }
    );
  }
