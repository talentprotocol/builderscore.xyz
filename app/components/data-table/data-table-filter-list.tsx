...
import { DataTableRangeFilter } from "@/app/components/data-table/data-table-range-filter";
...
import {
  ...
  FacetedList,
  FacetedTrigger,
} from "@/app/components/ui/faceted";
...
import {
  ...
  getFilterOperators,
} from "@/app/lib/data-table/data-table";
import { generateId } from "@/app/lib/data-table/id";
import { getFiltersStateParser } from "@/app/lib/data-table/parsers";
    ...
    case "dateRange": {
      ...
      const displayValue =
        filter.operator === "isBetween" && dateValue.length === 2
          ? `${formatDate(new Date(Number(dateValue[0])).toISOString())} - ${formatDate(
              new Date(Number(dateValue[1])).toISOString(),
            )}`
          : dateValue[0]
            ? formatDate(new Date(Number(dateValue[0])).toISOString())
            : "Pick a date";
